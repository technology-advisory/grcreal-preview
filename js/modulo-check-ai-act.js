// modulo-check-42001.js · Control de progreso para AI Act
// Clave única para este curso (no interfiere con ISO 27001)

// ============================================================
// CONFIGURACIÓN DEL CURSO
// ============================================================

const STORAGE_KEY = 'ai_act_completados';
const MODULOS_OBLIGATORIOS = [1, 2, 3, 4, 5, 6];

console.log(`🔑 Curso AI Act detectado. Clave de progreso: ${STORAGE_KEY}`);
...
function bloquearModulosIndex() {
    ...
    moduleCards.forEach(card => {
        ...
        if (numberSpan) {
            ...
            // Módulo 11 (resumen): requiere los 6 módulos completados
            if (num === 11) {
                if (completados.length < 6) {
            ...
            // Módulo 12 (consejos examen): siempre accesible después del resumen (o libre)
            if (num === 12) {
                // Para consejos, permitimos acceso si hay al menos 10 completados
                if (completados.length >= 6) {
                    card.classList.remove('locked');
                    const lockMsg = card.querySelector('.locked-message');
                    if (lockMsg) lockMsg.remove();
...
function checkAccess() {
    ...
    if (path.includes('consejos.html') || path.includes('modulo-12.html')) {
        ...
        if (completados.length < 6) {
            const firstPending = getFirstPendingModulo();
            alert(`❌ Para acceder a los consejos del examen, primero debes completar los 6 módulos.\n\nTe falta completar el módulo ${firstPending}.`);
            window.location.href = `modulo-${firstPending}.html`;
            return false;
        }
    }
...
function inyectarTimerLectura() {
    ...
    const ctaBlock = document.querySelector('.cta-buttons')?.closest('.block') || document.querySelector('.cta-buttons');
    if (!ctaBlock) return;
